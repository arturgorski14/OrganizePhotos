import pytest
from organize_photos import OrganizePhotos, GroupingLevel
...
class TestDetermineProgressBarStepIncrement:
    ...
    def setup_app(self, tmp_path):
        ...
        app = OrganizePhotos(folder_path=str(tmp_path), grouping_level=GroupingLevel.YYYY)
        return app
    ...
    def test_progress_bar_step_increment(self, setup_app, files_count, expected_increment):
        app = setup_app
        # Call the private method
        result = app._OrganizePhotos__determine_progress_bar_step_increment(files_count)

        # Check if the result matches the expected value
        assert result == expected_increment, f"Expected {expected_increment}, got {result}"
